      ...
      try {
        ...
        if (token !== null) {
          fetch(`https://book-swapper-backend.herokuapp.com/user_library`, {
            ...
            .then(data => {
              this.setState({
                userLibrary: data.books,
                tradeOffers: data.trades
              });
            });
        } else {
        ...
        buttonIndex => {
          if (buttonIndex === 1) {
            ...
            this.props.navigation.navigate("Login", {
              autoLogin: this.props.navigation.state.params.autoLogin
            });
          }
        }
      );
      ...
      <View style={styles.container}>
        ...
        <ScrollView>
          ...
          <View style={styles.bookshelf}>
            {this.state.userLibrary.length < 1 ? (
              <Text style={styles.emptyLibrary}>Your Library Is Empty</Text>
            ) : null}
            <FlatList
              horizontal={true}
              ...
              renderItem={({ item }) => {
                return (
                  <Book
                    key={item.props.book.id}
                    book={item.props.book}
                    title={item.props.book.title}
                    author={item.props.book.author}
                    ISBN={item.props.ISBN}
                    img={item.props.book.img}
                    description={item.props.book.description}
                    publisheddate={item.props.book.publishedDate}
                    pagecount={item.props.book.pageCount}
                    rating={item.props.book.rating}
                    infolink={item.props.book.infoLink}
                    googleid={item.props.book.googleid}
          ...
          <View style={styles.bookshelf}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                return (
                  <View style={styles.tradeOfferContainer}>
                    <Text style={{ fontSize: 15, textAlign: "center" }}>
                      {item.owned_book.title}
                    </Text>
                    <Text style={{ fontWeight: "bold", fontSize: 10 }}>
                      For:
                    </Text>
                    <Text style={{ fontSize: 15, textAlign: "center" }}>
                      {item.requestee.title}
                    </Text>
                    <TouchableOpacity
                      onPress={() => this.handleAcceptingTrade(item)}
                    >
